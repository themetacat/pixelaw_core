...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_OFFCHAIN_TABLE, bytes14(""), bytes16("Alert")))
);
ResourceId constant AlertTableId = _tableId;

FieldLayout constant _fieldLayout = FieldLayout.wrap(
  0x0050050104042014140000000000000000000000000000000000000000000000
);
...
library Alert {
   ...
   * @notice Set caller.
   */
  function setCaller(bytes32 id, address caller) internal {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = id;

    StoreSwitch.setStaticField(_tableId, _keyTuple, 3, abi.encodePacked((caller)), _fieldLayout);
  }

  /**
   * @notice Set caller.
   */
  function _setCaller(bytes32 id, address caller) internal {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = id;

    StoreCore.setStaticField(_tableId, _keyTuple, 3, abi.encodePacked((caller)), _fieldLayout);
  }
   ...
   * @notice Set player.
   */
  function _setPlayer(bytes32 id, address player) internal {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = id;
  ...
  function set(
    ...
    bytes memory _staticData = encodeStatic(x, y, timestamp, caller, player);
   ...
   */
  function set(bytes32 id, AlertData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.x, _table.y, _table.timestamp, _table.caller, _table.player);
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(bytes32 id, AlertData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.x, _table.y, _table.timestamp, _table.caller, _table.player);
  ...
  function decodeStatic(
    bytes memory _blob
  ) internal pure returns (uint32 x, uint32 y, uint256 timestamp, address caller, address player) {
    x = (uint32(Bytes.slice4(_blob, 0)));

    y = (uint32(Bytes.slice4(_blob, 4)));

    timestamp = (uint256(Bytes.slice32(_blob, 8)));

    caller = (address(Bytes.slice20(_blob, 40)));

    player = (address(Bytes.slice20(_blob, 60)));
  }
  ...
  ) internal pure returns (AlertData memory _table) {
    (_table.x, _table.y, _table.timestamp, _table.caller, _table.player) = decodeStatic(_staticData);
  ...
  function encodeStatic(
    ...
    return abi.encodePacked(x, y, timestamp, caller, player);
  }
